...
if __name__ == '__main__':
    ...
    for K in K_list:
        ...
        ## TODO determine which run was "best" in validation likelihood
            # print(f"------cur_score = {cur_score}, best_score = {best_score}")
            if cur_score > best_score:
                print("!!!!change best_seed")
                best_score = cur_score
                best_seed = seed

        # print(f"*******K = {K}, best_seed = {best_seed}")
        ## TODO assess best model on test data
        best_gmm = GMM_PenalizedMLEstimator_EM(
                K=K, D=D, seed=best_seed,
                max_iter=max_iter,
                variance_penalty_mode=25., variance_penalty_spread=100.0)
        best_gmm.fit(x_train_ND)
        test_score_per_pixel = best_gmm.score(x_test_ND) / (N * D)
        print(f'****Best model for K = {K}, best_seed = {best_seed}, test_score_per_pixel = {test_score_per_pixel}')
