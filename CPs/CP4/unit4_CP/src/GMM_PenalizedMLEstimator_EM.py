...
class GMM_PenalizedMLEstimator_EM(GMM_PenalizedMLEstimator):
    ...
    * Initialization method generate_initial_parameters() from parent as well
    ...
    def estep__calc_r_NK(self, x_ND):
        ...
        for n in range (N):
            r_NK[n] = np.exp(self.log_pi_K + np.sum(norm.logpdf(x_ND[n], self.mu_KD, self.stddev_KD), axis = 1) - logsumexp(self.log_pi_K + np.sum(norm.logpdf(x_ND[n], self.mu_KD, self.stddev_KD), axis = 1)))
    ...
    def mstep__update_log_pi_K(self, r_NK):
        ...
        log_pi_K = np.log(np.mean(r_NK + 1e-10, axis = 0))
    ...
    def mstep__update_stddev_KD(self, r_NK, x_ND):
        ...
        for k in range(self.K):
            dem = 1.0 / (self.variance_penalty_spread * self.variance_penalty_mode ** 2) + np.sum(r_NK[:, k])
            nem = 1 / (self.variance_penalty_spread * self.variance_penalty_mode) + np.sum(r_NK[:, k][:, np.newaxis] * np.square(x_ND - self.mu_KD[k]), axis=0)
            var = nem / dem
            stddev_KD[k] = np.sqrt(var)
        return stddev_KD
    ...
    def fit(self, x_ND, x_valid_ND=None, verbose=True):
        ...
        for iter_id in range(self.max_iter + 1):
            ...
            if x_valid_ND is None:
                ...
                va_score_per_pixel = self.score(x_valid_ND) / ND
