...
def run_forward_algorithm(log_pi_K, log_A_KK, log_lik_TK):
	...
	* quadratic in number of discrete hidden states K
	...
	for t in range(1, T):
		...
		# log_pdf_x_1toT += 0.0
		# alpha_TK[t, :] = 0.0
		# print("log_A_KK shape: ", log_A_KK.shape)
		# print("log_lik_TK shape: ", log_lik_TK[t].shape)
		# print("alpha_TK shape: ", alpha_TK.shape)


		log_denom = logsumexp(log_A_KK + log_lik_TK[t, :] + np.log(alpha_TK[t-1,:])[:, np.newaxis])
		log_pdf_x_1toT += log_denom
		alpha_TK[t, :] = np.exp( log_lik_TK[t, :] + logsumexp( np.log(alpha_TK[t-1, :])[:, np.newaxis] + log_A_KK, axis = 0) - log_denom )	
