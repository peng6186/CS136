...
def calc_joint_log_pdf(z_D, t_N, phi_NM, w_prior, v_prior):
    ...
    mean_N, stddev_N = unpack_mean_N_and_stddev_N(z_D = z_D, phi_NM = phi_NM)
    log_pdf_t = sum(norm.logpdf(t_N, mean_N, stddev_N))


    a = 10.0 # set a = 10 in the cd3 description
    # log_pdf_w = 0.0 # TODO compute prior log p(w)
    w_mean = np.zeros(M)
    w_mean[0] = 10.0
    w_std = a * np.ones(M)
    log_pdf_w = sum(norm.logpdf(w_M, w_mean, w_std))
    
    # log_pdf_v = 0.0 # TODO compute prior log p(v)
    v_mean = np.zeros(M)
    v_mean[0] = 10.0
    v_std = a * np.ones(M)
    log_pdf_v = sum(norm.logpdf(v_M, v_mean, v_std))
...
if __name__ == '__main__':
    ...
    print("x_tr_N1.shape:", x_tr_N1.shape)
    print("t_tr_N.shape:", t_tr_N.shape)

    order = 2
    B = 2000 # Num Burnin samples
    ...
    N, M = phi_tr_NM.shape
    print("phi_tr_NM:", phi_tr_NM.shape)
    ...
    for d in range(D):
        list_of_rw_stddev_D[d][d] = 0.2 # TODO TUNE ME FOR EACH DIM
