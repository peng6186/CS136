...
def calc_target_log_pdf(z_D):
    ...
    # -0.5 * np.sum(np.square(z_D)) 
    mul_dis = multivariate_normal(mean=[-1.0, 1.0], cov=[[2.0, 0.95], [0.95, 1.0]])
    return mul_dis.logpdf(z_D)
...
if __name__ == '__main__':
    ...
    for rr, rw_stddev in enumerate(rw_stddev_grid):
        ...
        sampler = RandomWalkSampler(calc_tilde_log_pdf= calc_target_log_pdf, list_of_rw_stddev_D= [rw_stddev_D]
            , random_state=random_state)
        z_listA, sample_infoA = sampler.draw_samples(zinit_D = z_initA_D, n_keep_samples= n_keep_samples, n_burnin_samples= n_burnin_samples)
        z_listB, sample_infoB = sampler.draw_samples(zinit_D = z_initB_D, n_keep_samples= n_keep_samples, n_burnin_samples= n_burnin_samples)
