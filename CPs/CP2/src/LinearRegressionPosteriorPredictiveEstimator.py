...
class LinearRegressionPosteriorPredictiveEstimator():
    ...
    def fit(self, x_ND, t_N):
        ...
        self.mean_M = np.linalg.solve(np.matmul(Phi_NM.transpose(), Phi_NM) + self.alpha / self.beta * np.identity(self.M), np.matmul(Phi_NM.transpose(), t_N))
        self.precision_MM = self.alpha * np.identity(self.M) + self.beta * np.matmul(Phi_NM.transpose(), Phi_NM)
    ...
    def predict(self, x_ND):
        ...
        return np.matmul(phi_NM, self.mean_M)
    ...
    def predict_variance(self, x_ND):
        ...
        res = 1.0 * np.ones(N)
        for i in range(N):
            res[i] = 1.0 / self.beta + np.matmul(np.matmul(phi_NM[i], np.linalg.inv(self.precision_MM)), phi_NM[i].transpose())

        # print("N:", N)
        # ret = 1.0 / self.beta + np.matmul(np.matmul(phi_NM, self.precision_MM), phi_NM.transpose())
        # print("var_shape:", res.shape)
        # print("var:", res)
        return res
        # return 1.0 / self.beta + np.matmul(np.matmul(phi_NM, self.precision_MM), phi_NM.transpose())
    ...
    def fit_and_calc_log_evidence(self, x_ND, t_N):
        ...
        # print("within calc_log_evd, mean_M: ", self.mean_M)
        # print("within calc_log_evd, precision_MM: ", self.precision_MM)
        ## TODO perform evidence calculation
        ret = M / 2 * np.log(self.alpha) + N / 2 * np.log(self.beta) - (self.beta / 2 * np.inner(t_N - phi_NM@self.mean_M, t_N - phi_NM@self.mean_M) + self.alpha / 2 * self.mean_M.T @ self.mean_M) - 1 / 2 * np.log(np.linalg.det(self.precision_MM)) - N / 2 * np.log(2 * np.pi)
        # print("before return ret, ret: ", ret)
        return ret
