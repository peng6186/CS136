...
class LinearRegressionMAPEstimator():
    ...
    def fit(self, x_ND, t_N):
        ...
        # update w_map_M attribute via formulas from Bishop (Okay)
        self.w_map_M = np.zeros(M)
        # print("Phi_NM:", Phi_NM)
        # self.w_map_M = np.matmul(np.linalg.inv(np.matmul(Phi_NM.transpose(), Phi_NM) + self.alpha / self.beta * np.identity(M)), np.matmul(Phi_NM.transpose(), t_N))
        self.w_map_M = np.linalg.solve(np.matmul(Phi_NM.transpose(), Phi_NM) + self.alpha / self.beta * np.identity(M), np.matmul(Phi_NM.transpose(), t_N))
        # print("w_map_M:", self.w_map_M)
        return self
    ...
    def predict(self, x_ND):
        ...
        phi_NM = self.feature_transformer.transform(x_ND)
        N, M = phi_NM.shape
        ## compute mean predictionƒ
        return np.matmul(phi_NM, self.w_map_M)
