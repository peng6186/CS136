    ...
    * 2) does not give zero probability to any word
    ...
    def fit(self, word_list):
        ...
        * The integer total_count is set to the total length of the word list
        ...
        for count in self.count_V:
            if count == 0:
                self.unseen_count += 1
        # print("total_count: ", self.total_count)
        # print("count_V: ", self.count_V)
        # print("unseen_count:", self.unseen_count)
    ...
    def predict_proba(self, word):
        ...
        word_id = self.vocab.get_word_id(word)
        word_count = self.count_V[word_id]

        if self.unseen_count == 0: # all words appeared
            return word_count / self.total_count
        else: # some words are not present
            if word_count == 0:
                return self.epsilon_unseen_proba / self.unseen_count
            else:
                return (1 - self.epsilon_unseen_proba) * word_count / self.total_count
