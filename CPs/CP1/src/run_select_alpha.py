...
def calc_per_word_log_evidence(estimator, word_list):
    ...
    V = len(estimator.count_V)
    N = len(word_list)
 
    prod_nv_alpha = 0
    prod_alpha = 0
    for i in range (V):
        prod_nv_alpha += gammaln( estimator.count_V[i] + estimator.alpha)
        prod_alpha += gammaln(estimator.alpha)


    log_evidence = gammaln(V * estimator.alpha) + prod_nv_alpha - gammaln(N + V * estimator.alpha) - prod_alpha
    # print("V:", V)
    # print("N:", N)
    # print("prod_nv_alpha:", prod_nv_alpha)
    # print("prod_alpha:", prod_alpha)
...
if __name__ == '__main__':
    ...
    for nn, N in enumerate(n_train_list):
        ...
        training_data = train_word_list[:N]
        for idx, alpha in enumerate(alpha_list):
            ppe = PosteriorPredictiveEstimator(vocab=vocab, alpha=alpha)
            train_log_ev_list[idx] = calc_per_word_log_evidence(ppe, training_data)
            test_log_lik_list[idx] = ppe.score(test_word_list)
        ...
        ax_grid[nn].set_xlabel('the value of alpha')
        if nn == 0:
            ax_grid[nn].set_ylabel('per-word log probability')
    plt.tight_layout()
    plt.show()
